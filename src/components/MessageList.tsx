...
export default function MessageList({ jobId }: { jobId: string }) {
  const { user } = useAuth();
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (content: string) => {
      if (!user?.id || !jobId) throw new Error('Missing user or job ID');
      
      console.log('Sending message:', { jobId, userId: user.id, content });
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['conversations-dropdown'] });
    },
    onError: (error) => {
      console.error('Error sending message:', error);
    },
